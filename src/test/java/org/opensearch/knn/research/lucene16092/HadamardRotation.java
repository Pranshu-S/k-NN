...
/*
 * ============================================================================
 * PROVENANCE — this is a VERBATIM copy of the rotation implementation from
 * Apache Lucene pull request #16092 ("Add hadamard rotation to vector fields").
 *
 *   source repo   : https://github.com/apache/lucene
 *   PR            : https://github.com/apache/lucene/pull/16092
 *   PR head SHA   : 132f8b17715db53a54608a0bbc8cb75b16be7f87
 *   commit date   : 2026-08-13 (fetched 2026-08-18)
 *   source path   : lucene/core/src/java/org/apache/lucene/util/quantization/HadamardRotation.java
 *   source sha256 : 1044cf4edd1c21909f8c2745b8c39c308b5fe0c1ba6a853ba5aef0fd842a6e7c
 *
 * The ONLY changes from the upstream file are non-algorithmic:
 *   (1) the package declaration (org.apache.lucene.util.quantization ->
 *       org.opensearch.knn.research.lucene16092) so it can live in this repo's
 *       test tree without a split package with the lucene-core jar;
 *   (2) one javadoc {@link org.apache.lucene.codecs.PreconditioningKnnVectorsFormat}
 *       cross-reference downgraded to plain text (that class is not on this
 *       classpath). No executable code is altered.
 *
 * The transform (random sign flips -> Fisher-Yates permutation -> block-diagonal
 * normalized FWHT) is byte-for-byte identical to the PR. Verify with:
 *   diff <(sed '/^package /d' this_file) <(sed '/^package /d' upstream_file)
 * ============================================================================
 */
...
public final class HadamardRotation {
   ...
   * Exposed so callers (e.g. the codec preconditioning format) can persist the seed alongside the
   * rotated vectors, decoupling on-disk indices from any future change to the seed-mix function:
   * future readers reconstruct the rotation from the persisted seed via {@link #create(int, long)}
   * rather than re-deriving from the dimension.
   */
